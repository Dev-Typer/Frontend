...
const ME_BEST_SNIPPETS: BestSnippet[] = [
  ...
  { title: 'Data class',         lang: 'Kotlin',     diff: 'easy',   core: 108, rank: 29, total: 980  },
  ...
  { title: 'Sealed class',       lang: 'Kotlin',     diff: 'medium', core: 105, rank: 33, total: 980  },
  ...
  { title: 'Result<T, E>',       lang: 'Rust',       diff: 'hard',   core: 101, rank: 36, total: 1640 },
  ...
  { title: 'Extension fn',       lang: 'Kotlin',     diff: 'medium', core: 95,  rank: 43, total: 980  },
  ...
  { title: 'tailrec fn',         lang: 'Kotlin',     diff: 'hard',   core: 91,  rank: 48, total: 980  },
  { title: 'Defer cleanup',      lang: 'Go',         diff: 'easy',   core: 90,  rank: 45, total: 1870 },
  { title: 'select stmt',        lang: 'Go',         diff: 'medium', core: 90,  rank: 49, total: 1870 },
  { title: 'String parse',       lang: 'C',          diff: 'easy',   core: 89,  rank: 46, total: 1780 },
  { title: 'when expr',          lang: 'Kotlin',     diff: 'easy',   core: 88,  rank: 47, total: 980  },
  { title: 'Nullable struct',    lang: 'C#',         diff: 'easy',   core: 88,  rank: 53, total: 1920 },
  { title: 'Template fn',        lang: 'C++',        diff: 'medium', core: 86,  rank: 51, total: 2090 },
  { title: 'Pattern match',      lang: 'Rust',       diff: 'medium', core: 84,  rank: 55, total: 1640 },
  { title: 'LINQ select',        lang: 'C#',         diff: 'easy',   core: 82,  rank: 58, total: 1920 },
...
export const ME: User = {
  ...
  byLang: [
    ...
    { lang: 'Kotlin',     core: 280,  snippets: 6,  wpm: 73, plays: 16,  top: { title: 'Coroutine flow',    diff: 'hard',   core: 113, rank: 22, total: 980  } },
  ],
};
...
export const SOLO_TRACKS: SoloTrack[] = [
  ...
  { id: 'cpp-raii', lang: 'c++', difficulty: 'hard', title: 'RAII guard', avgWpm: 59,
    code: `class FileGuard {\npublic:\n    explicit FileGuard(const std::string& path)\n        : file_(std::fopen(path.c_str(), "r")) {}\n    ~FileGuard() { if (file_) std::fclose(file_); }\n    FILE* get() const { return file_; }\nprivate:\n    FILE* file_ = nullptr;\n};` },
  { id: 'cpp-template', lang: 'c++', difficulty: 'medium', title: 'template clamp', avgWpm: 65,
    code: `template<typename T>\nT clamp(T val, T lo, T hi) {\n    return val < lo ? lo : val > hi ? hi : val;\n}` },
  { id: 'csharp-linq', lang: 'c#', difficulty: 'medium', title: 'LINQ chain', avgWpm: 68,
    code: `var result = users\n    .Where(u => u.IsActive)\n    .OrderByDescending(u => u.Score)\n    .Take(10)\n    .Select(u => new { u.Name, u.Score })\n    .ToList();` },
  { id: 'csharp-async', lang: 'c#', difficulty: 'hard', title: 'async Task', avgWpm: 60,
    code: `public async Task<string?> FetchAsync(string url) {\n    using var client = new HttpClient();\n    var res = await client.GetAsync(url);\n    if (!res.IsSuccessStatusCode) return null;\n    return await res.Content.ReadAsStringAsync();\n}` },
  { id: 'c-linked', lang: 'c', difficulty: 'medium', title: 'reverse list', avgWpm: 65,
    code: `typedef struct Node { int val; struct Node *next; } Node;\n\nNode *reverse(Node *head) {\n    Node *prev = NULL, *curr = head, *next;\n    while (curr) {\n        next = curr->next;\n        curr->next = prev;\n        prev = curr;\n        curr = next;\n    }\n    return prev;\n}` },
  { id: 'c-bits', lang: 'c', difficulty: 'easy', title: 'count bits', avgWpm: 80,
    code: `int count_bits(unsigned int n) {\n    int c = 0;\n    while (n) { c += n & 1; n >>= 1; }\n    return c;\n}` },
  { id: 'rust-result', lang: 'rust', difficulty: 'medium', title: 'Result<T,E>', avgWpm: 64,
    code: `use std::num::ParseIntError;\n\nfn parse_double(s: &str) -> Result<i32, ParseIntError> {\n    let n = s.trim().parse::<i32>()?;\n    Ok(n * 2)\n}` },
  { id: 'rust-trait', lang: 'rust', difficulty: 'hard', title: 'impl Trait', avgWpm: 56,
